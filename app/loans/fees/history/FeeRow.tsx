...
export default function FeeRow({ fee }: { fee: any }) {
  ...
  return (
    <>
      <tr className="border-t hover:bg-gray-50/50 border-white/40 transition">
        {/* Customer */}
        <td className="px-4 py-3">
          <div className="font-medium">{fee.customer?.applicantName}</div>
          <div className="text-xs text-gray-500">
          ...
          {fee.customer?.memberId && (
            <div className="text-[11px] text-gray-400 mt-0.5">
              Member:{' '}
              <span className="font-mono">{fee.customer.memberId}</span>
            </div>
          )}
        </td>
        ...
        {/* Empty column (kept for alignment if needed) */}
